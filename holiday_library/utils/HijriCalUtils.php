...
class HijriCalendar {
	...
	function getFirstDayOfHijriMonth($hijriMonth, $miladyYear)
	{
		$retVal = array();
		$dUtils = new DateUtils();
		$hijriYear = $this->dateInfo(30,6,$miladyYear);
		$hijriYear = $hijriYear["hijriYear"] - 1;
		for($i=0; $i<3; $i++) {
			$temp = $this->hijriToMiladi(1, $hijriMonth, $hijriYear+$i);
			$d = new EnricoDate($temp["day"],$temp["month"],$temp["year"]);
			$d = $dUtils->addDays($d, -2);
			for($k=0; $k<5; $k++)
			{
				$dateInfo = $this->dateInfo($d->day, $d->month, $d->year);
				if($dateInfo["hijriDay"]==1 && $dateInfo["hijriMonth"]==$hijriMonth && $d->year == $miladyYear) {
					 array_push($retVal, $d);
					 break;
				}
				$d = $dUtils->addDays($d, 1);
			}
		}
